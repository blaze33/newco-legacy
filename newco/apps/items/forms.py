...
class ItemFormComplex(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "instance" not in kwargs or kwargs["instance"] is None:
            self.create = True
            self.user = self.request.user
        return super(ItemFormComplex, self).__init__(*args, **kwargs)

    def save(self, commit=True, **kwargs):
        if commit and self.create:
            item = super(ItemFormComplex, self).save(commit=False)
            item.author = self.user
            item.save()
            self.save_m2m()
        else:
            item = super(ItemFormComplex, self).save(commit)
...
class QuestionFormComplex(ModelForm):

    create = False
    no_results = _("No results matched")

    additional_field_test = CharField('additional_field_test')

    class Meta:
        model = Question
        fields = ("content", "status", "additional_field_test")# "items")
        widgets = {
            "content": Textarea(attrs={
                "class": "span6",
                "placeholder": _("Ask something specific."),
    ...
    def __init__(self, *args, **kwargs):
        if 'instance' not in kwargs or kwargs['instance'] is None:
            ...
            self.request = kwargs.pop('request')
            self.user = self.request.user
        super(QuestionFormComplex, self).__init__(*args, **kwargs)
        self.fields['additional_field_test2'] = CharField('additional_field_test2')
        # self.fields.append('additional_field_test2')
        # self.fields["items"].help_text = _(
        #     "Select one product using Enter and the Arrow keys")
        # if hasattr(self, "request"):
        #     if self.request.GET.get("fields", "") != "add_items":
    ...
    def save(self, commit=True, **kwargs):
        if commit and self.create:
            question = super(QuestionFormComplex, self).save(commit=False)
            question.author = self.user
            question.save()
            if "items" in self.fields:
                self.save_m2m()
            ...
            return super(QuestionFormComplex, self).save(commit)
...
class AnswerFormComplex(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        if "instance" not in kwargs or kwargs["instance"] is None:
            ...
            self.object = kwargs["instance"]
            self.question = self.object.question
        super(AnswerFormComplex, self).__init__(*args, **kwargs)
        self.fields['content'].label = _("Your answer")

    def save(self, commit=True, **kwargs):
        if commit and self.create:
            answer = super(AnswerFormComplex, self).save(commit=False)
            answer.author = self.user
            answer.question = self.question
            answer.save()
            answer.items = answer.question.items.all()
            ...
            return super(AnswerFormComplex, self).save(commit)
